...
## 4.2 Factors influcencing maintenance methods
...
```
The contingency table table **"(trouble_data$BodyStyles, trouble_data$Methods)"** lists the raw counts of how many times each maintenance method occurs for each body style. Raw counts let us see absolute workload differences, which is useful for statistical test and the visualization later on.
...
In order for us to better visualise the porpotions of maintenance methods over body style, it is best to use a filled bar chart. 
...
git add .
git commit -m "Task 3.3: Filter out those engines in the dataset that have trouble or are suspected of having trouble"
git add .
...
git add .
git commit -m "Task 4.2: 2 Factors influcencing maintenance methods"
git push
